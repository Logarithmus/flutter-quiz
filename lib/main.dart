...
class Question {
  ...
  Question.fromSnapshot(DataSnapshot snapshot)
      : question = snapshot.value['question'],
        correctAnswerIndex = snapshot.value['creationTimestamp'],
        answers = List.from(snapshot.value['answers']);
}
        ...
        home: FutureBuilder(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                print(
                    'You have an error in Firebase: ${snapshot.error.toString()}');
                return Text("Something went wrong with Firebase");
              } else if (snapshot.hasData) {
                return MyHomePage(title: 'QuizFlutter Home Page');
              } else {
                return Center(
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _loadCounter() {
      _counterRef.once().then((DataSnapshot snap) {
        setState(() {
          _counter = snap.value;
        });
      });
